...
int Qtx::exec()
{
    if (container->needToExit)
    {
        AppExitEvent *exitApplicationEvent = new AppExitEvent;
        app->postEvent(qApp, exitApplicationEvent, Qt::HighEventPriority);
    }
    eventLoopStarted = true;
    return app -> exec();
}

void Qtx::exitApp(int codeResult)
{
    if (eventLoopStarted)
    {
        app->exit(codeResult);
...
void Qtx::msleep(unsigned long ms)
{
    if (!eventLoopStarted && container->needToExit)
    {
        exitApp(0);
        return;
    }
    QTime time;
    time.start();
    for(;(unsigned long)time.elapsed() < ms;)
    {
        app -> processEvents();
    }
}
...
Qtx::Qtx(char *s, char **argv_)
{
    eventLoopStarted = false;
    needToExitAppWithoutEventLoop = false;
    int i = 0;
    painter = NULL;
    pixmap = NULL;
    container = NULL;
...
void Qtx::notifyIcon(const char *title, const char *text, const char *filename)
{
...
bool Qtx::applicationMustBeClosed()
{
    return container->needToExit;
}
...
const char *Qtx::applicationDirectoryPath()
{
    QByteArray ba = qApp->applicationDirPath().toLocal8Bit();
    const int szBa = ba.size();
    char* retVal = new char[szBa+1];
    strcpy(retVal, ba.constData());
    const char *cRetVal = retVal;
    return cRetVal;
}

const char *Qtx::fromApplicationDirectoryPath(const char *subDirPath)
{
    QString appDirP = qApp->applicationDirPath();
    QString subDirectoryP = QString("/") + QString(subDirPath);
    appDirP.append(subDirectoryP);
    QByteArray ba = appDirP.toLocal8Bit();
    const int szBa = ba.size();
    char* retVal = new char[szBa+1];
    strcpy(retVal, ba.constData());
    const char *cRetVal = retVal;
